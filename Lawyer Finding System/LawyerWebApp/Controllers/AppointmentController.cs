    ...
    public class AppointmentController : Controller
    {
       ...
       AppointmentRepository appointmentRepository = new AppointmentRepository();
        ...
        public ActionResult Create(int userID, Appointment answer)
        {
            ...
            List<Appointment> appointmentList = lawyerDBEntities.Appointments.ToList();
            List<int?> lawyerList = appointmentList.Select(m => m.LawyerID).ToList();
            List<int?> list = new List<int?>();
            bool flag = true;

            for (int i = 0; i < lawyerList.Count; i++)
            {
                if (lawyerList[i].Equals(answer.LawyerID))
                {
                    flag = true;
                }
                else
        ...
        [HttpPost]
        public ActionResult Delete(int id, Appointment lawyer)
        {
            try
            {
                // TODO: Add update logic here
