...
namespace LawyerWebApp.Controllers
{
    public class AnswerController : Controller
    {
        ...
        public ActionResult KMPSubstringSearchMethod(string txtSearch)
        {
            ...
            List<Answer> questionList = lawyerDBEntities.Answers.ToList();
            List<string> questions = questionList.Select(o => o.Question.Question1).ToList();
            List<string> list = new List<string>();


            for (var p = 0; p < questions.Count; p++)
            {

                var firstOrDefault = questions[p];


                string text = firstOrDefault;
            ...
            return View(lawyerDBEntities.Answers.Where(c => list.Contains(c.Question.Question1)).ToList());
